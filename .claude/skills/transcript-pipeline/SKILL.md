...
One manual trigger that chains three skills and ends with a phone notification:

```
snapshot index → zoom-transcript-sync → diff index
   ├─ nothing new → notify "No new transcripts" → stop
   └─ new files   → discovery-debrief → transcript-brief → notify brief
```
...
- **Never write to `~/transcripts/index.json`.** Not to fix it, not to annotate a run,
...
Read `~/transcripts/index.json` and keep in memory the set of entries that have a
file:
...
Invoke the `zoom-transcript-sync` skill and follow it fully. The destination is
`~/transcripts`; do not ask.
...
authenticated, a Drive listing that errors, or the sync stopping before it rewrote the
index. On a real failure go to step 7 with step `sync`.
...
Check each file in `new_files` actually exists in `~/transcripts`. Keep the list in
...
exactly `new_files`. It writes `ledger.md` and prints its full A/B/C analysis into
...
If the debrief fails, go to step 7 with step `debrief`.
...
`Thesis:` line. Print the full brief in the session.

If the brief fails, go to step 7 with step `brief`.
...
Phone delivery needs Remote Control connected for this session. If it is not, the
notification appears only on the desktop; mention this once at the end of the run.
...
`<N> new files saved.` so the user knows the sync part worked and the next run's
...
End the run in the session with three lines: what was synced, what the debrief
changed in the ledger, and the notification text that was sent (or why it was not).
